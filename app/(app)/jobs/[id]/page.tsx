...
export default async function JobDetailPage({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Actions, §5 order: Add expense · Add line item · Send estimate ·
          Send invoice · Mark done */}
      <div className="flex flex-col gap-3">
        <LinkButton href={`/jobs/${job.id}/expense`}>+ Add expense</LinkButton>
        <LinkButton href={`/jobs/${job.id}/line-item`}>+ Add line item</LinkButton>
        <form action={sendEst} className="contents">
          <Button type="submit" className="w-full bg-teal-700">Send estimate</Button>
        </form>
        <LinkButton href={`/jobs/${job.id}/invoice`} className="bg-teal-700">
          Send invoice
        </LinkButton>
      ...
      <Card>
        <div className="mb-2 flex items-center justify-between">
          ...
          <form action={permit}>
            <Button type="submit" className="min-h-12 bg-teal-700 px-4 text-base">+ Add permit</Button>
          </form>
        </div>
      ...
      <Card>
        ...
        <ul className="flex flex-col gap-2">
          {job.invoices.map((inv) => {
            ...
            return (
              <li key={inv.id}>
                <Link href={`/invoices/${inv.id}`} className="flex min-h-12 items-center justify-between gap-2">
