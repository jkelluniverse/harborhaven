...
import { sendInvoiceAction, cancelInvoiceAction, textInvoiceLinkAction } from "@/lib/actions";
...
export default async function InvoiceDetailPage({
  ...
  const textLink = textInvoiceLinkAction.bind(null, invoice.id);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <p className="text-lg text-stone-700">
          <Link href={`/app/jobs/${invoice.jobId}`} className="text-teal-800 underline">
      ...
      {invoice.publicUrl && invoice.job.client.phone && invoice.status !== "PAID" && invoice.status !== "CANCELED" && (
        <form action={textLink} className="contents">
          <button
            type="submit"
            className="min-h-12 rounded-xl border border-teal-800 px-5 text-lg font-semibold text-teal-800"
          >
            Text the payment link to {invoice.job.client.name.split(" ")[0]}
          </button>
        </form>
      )}
